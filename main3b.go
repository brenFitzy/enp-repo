...
//      } else {
        ...
        if len(a.ManufacturerData()) > 0 {
			...
			SoDatasval := new(SOutput)
				...
				Osdata = append(Osdata, SOutput{
				 Sotimedate: time.Now(),
				 Soname: Datasval.Sname,
				 Solocation : Datasval.Slocation,
				 Sotype: Datasval.Stype,
				 Sodatamac: Datasval.Smac,
				 Sodatamid: string(((Datasval.Sdata.ValSdatabytes["MSBmid"]) | (Datasval.Sdata.ValSdatabytes["LSBmid"]))),
				 SodataPm1: ((Datasval.Sdata.ValSdatabytes["MSBPm01"]) + (Datasval.Sdata.ValSdatabytes["LSBPm01"])),
				 SodataPm25: ((Datasval.Sdata.ValSdatabytes["MSBPm25"]) + (Datasval.Sdata.ValSdatabytes["LSBPm25"])),
				 SodataPm10: ((Datasval.Sdata.ValSdatabytes["MSBPm10"]) + (Datasval.Sdata.ValSdatabytes["LSBPm10"])),
				 Sodatatemp:  (float32(((Datasval.Sdata.ValSdatabytes["MSBTemp"]) | (Datasval.Sdata.ValSdatabytes["LSBTemp"] *256)))/100),
				 Sodatahumd: (float32(((Datasval.Sdata.ValSdatabytes["MSBHund"]) | (Datasval.Sdata.ValSdatabytes["LSBHund"] *256)))/100),
				 Sodatavoc: ((Datasval.Sdata.ValSdatabytes["MSBVoc"]) | (Datasval.Sdata.ValSdatabytes["LSBVoc"] *256)),
				 Sodataco2: ((Datasval.Sdata.ValSdatabytes["MSBCo2"]) | (Datasval.Sdata.ValSdatabytes["LSBCo2"] *256)),
				 })
				 fmt.Printf("append", Osdata)
